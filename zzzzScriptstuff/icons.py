...
# Clock spinner sequence. Nerd Fonts has hour-by-hour clock glyphs in the
# Material Design set, but their codepoints changed in NF v3 and many
# Konsole/font setups don't render them. Use a single, reliable clock
# glyph from Font Awesome (nf-fa-clock_o, U+F017) that's been stable
# across NF v1/v2/v3.
CLOCK_FRAMES = [
    "\uf017",  # nf-fa-clock_o — single static clock face
]
# For ASCII fallback we DO cycle (since braille spinners are well-supported)
CLOCK_FRAMES_ASCII = ["⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏"]

# Hourglass cycle — also broken at the previous codepoints. Use the
# stable Font Awesome ones.
SPINNER_FRAMES_NF = ["\uf252", "\uf253", "\uf254"]   # nf-fa-hourglass_* (half/three_quarters/start)
