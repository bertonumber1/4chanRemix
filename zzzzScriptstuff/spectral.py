...
def _verdict(res: Analysis) -> None:
    ...
    # MEASURED on a controlled round trip (one library FLAC, encoded with
    # libmp3lame and decoded straight back):
    #
    #     file        verdict   cutoff   wall    above
    #     original    clean     21.4k    1.7dB   -80.2dB
    #     320 kbps    suspect   19.0k    3.4dB  -106.6dB
    #     192 kbps    suspect   18.3k   16.8dB  -111.9dB
    #     128 kbps    lossy     16.6k   31.1dB  -101.3dB
    #
    # The all-three rule only catches 128. At 192 and 320 the wall is gentle,
    # so the strongest tell is not the shape of the edge but the SILENCE above
    # it: every transcode sits at -101 dB or lower while the real master keeps
    # a -80 dB dither floor. No 16-bit master is that quiet up there — the
    # dither alone puts a floor in. So an extreme dead band, with a real gap
    # below Nyquist, is a verdict on its own.
    silent_above = res.above_db <= -95.0
    if (silent_above and headroom >= 1500.0 and res.sample_rate <= 48000
            and not (sharp_wall and low_cutoff and dead_above)):
        res.verdict = "lossy"
        res.confidence = 85
        res.reasons = [
            f"cuts off at {res.cutoff_hz/1000:.1f} kHz, {headroom/1000:.1f} kHz below Nyquist",
            f"and only {res.above_db:.0f} dB above it — a real 16-bit master leaves "
            f"a dither floor around -80 dB up there, an encoder leaves nothing",
        ]
